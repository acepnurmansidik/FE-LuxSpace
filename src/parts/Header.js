...
const Header = ({ theme, posotion }) => {
  ...
  const prevChart = useRef(state?.cart || {});

  useLayoutEffect(() => {
    // cek jika cart nya tidak sama dengan yang di klik(Add to Cart)
    if (prevChart.current !== state.cart) {
      prevChart.current = state?.cart;
